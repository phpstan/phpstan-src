...
function test(): void
{
	...
	User::query()->whereHas('posts', function (PostBuilder $query) {
		return $query->where('name', 'test');
	});
...
{
	$userOrNull = rand() ? User::query() : null;
	$userOrNull?->whereHas('posts', function ($query) {
		assertType('Bug12585\PostBuilder', $query);
	});
}

function testNullOrObject(): void
{
	/** @var Builder<User>|null $builderOrNull */
	$builderOrNull = null;
	$builderOrNull?->whereHas('posts', function ($query) {
		assertType('Bug12585\PostBuilder', $query);
	});
}
...
{
	(new Builder(new User()))->whereHas('posts', function ($query) {
		assertType('Bug12585\PostBuilder', $query);
	});
}
...
{
	Builder::create(new User())->whereHas('posts', function ($query) {
		assertType('Bug12585\PostBuilder', $query);
	});
}

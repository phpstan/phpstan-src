...
final class ProcessPromise
{
	...
	public function run(): PromiseInterface
	{
		...
		if ($tmpStdOutResource === false) {
			throw new ProcessCrashedException('Failed creating temp file for stdout.');
		}
		$tmpStdErrResource = tmpfile();
		if ($tmpStdErrResource === false) {
			throw new ProcessCrashedException('Failed creating temp file for stderr.');
		}
	...
	private function cancel(): void
	{
		if ($this->process === null) {
			throw new ProcessCanceledException('Cancelling process before running');
		}
		$this->canceled = true;

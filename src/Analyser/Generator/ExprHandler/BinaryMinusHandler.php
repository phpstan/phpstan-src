...
/**
 * @implements ExprHandler<Expr\BinaryOp\Minus>
 */
#[AutowiredService]
final class BinaryMinusHandler implements ExprHandler
{
	...
	{
		return $expr instanceof Expr\BinaryOp\Minus;
	}
	...
	public function analyseExpr(Stmt $stmt, Expr $expr, GeneratorScope $scope, ExpressionContext $context, ?callable $alternativeNodeCallback): Generator
	{
		...
		return new ExprAnalysisResult(
			$this->initializerExprTypeResolver->getMinusTypeFromTypes($expr->left, $expr->right, $leftResult->type, $rightResult->type),
			$this->initializerExprTypeResolver->getMinusTypeFromTypes($expr->left, $expr->right, $leftResult->nativeType, $rightResult->nativeType),
			$rightResult->scope,

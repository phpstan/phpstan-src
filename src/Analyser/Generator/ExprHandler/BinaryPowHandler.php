...
/**
 * @implements ExprHandler<Expr\BinaryOp\Pow>
 */
#[AutowiredService]
final class BinaryPowHandler implements ExprHandler
{
	...
	{
		return $expr instanceof Expr\BinaryOp\Pow;
	}
	...
	public function analyseExpr(Stmt $stmt, Expr $expr, GeneratorScope $scope, ExpressionContext $context, ?callable $alternativeNodeCallback): Generator
	{
		...
		return new ExprAnalysisResult(
			$this->initializerExprTypeResolver->getPowTypeFromTypes($expr->left, $expr->right, $leftResult->type, $rightResult->type),
			$this->initializerExprTypeResolver->getPowTypeFromTypes($expr->left, $expr->right, $leftResult->nativeType, $rightResult->nativeType),
			$rightResult->scope,

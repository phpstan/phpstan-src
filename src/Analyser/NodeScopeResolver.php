	...
	): ExpressionResult
	{
		...
		if ($parameters !== null) {
			foreach ($args as $i => $arg) {
				...
				$assignByReference = $currentParameter !== null && $currentParameter->passedByReference()->createsNewVariable();
		...
		$scopeBeforeArgInvalidation = $scope;

		if ($parameters !== null && $calleeReflection !== null && $calleeReflection->hasSideEffects()->yes()) {
			foreach ($args as $i => $arg) {
				$currentParameter = null;
				if (isset($parameters[$i])) {
					$currentParameter = $parameters[$i];
		...
		return new ExpressionResult($scope, $hasYield, $isAlwaysTerminating, $throwPoints, $impurePoints, scopeBeforeArgInvalidation: $scopeBeforeArgInvalidation);
	}

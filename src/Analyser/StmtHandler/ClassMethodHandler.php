...
final class ClassMethodHandler implements StmtHandler
{
	...
	): InternalStatementResult
	{
		...
		if ($stmt->stmts !== null) {
			...
			try {
				$nodeScopeResolver->pushNodeGatherer(static function (Node $node, Scope $scope) use ($nodeScopeResolver, $methodScope, &$gatheredReturnStatements, &$gatheredReturnStatementsAfterFinally, &$gatheredYieldStatements, &$executionEnds, &$methodImpurePoints): void {

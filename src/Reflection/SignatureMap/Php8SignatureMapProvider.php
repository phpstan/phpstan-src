...
class Php8SignatureMapProvider implements SignatureMapProvider
{
	...
	{
		$signatures = [];
		foreach ($functionMapSignatures as $i => $functionMapSignature) {
			if ($i === 0) {
				$signatures[] = $this->mergeSignatures($nativeSignature, $functionMapSignature);
				continue;
			}

			$signatures[] = $functionMapSignature;
		}

...
use const CURLINFO_CONTENT_LENGTH_DOWNLOAD_T;
use const CURLINFO_CONTENT_LENGTH_UPLOAD;
use const CURLINFO_CONTENT_LENGTH_UPLOAD_T;
...
final class CurlGetinfoFunctionDynamicReturnTypeExtension implements DynamicFunctionReturnTypeExtension
{
	...
	private function cacheReturnTypes(): void
	{
		...
		$this->componentTypesPairedConstants = [
			...
			CURLINFO_CONTENT_LENGTH_DOWNLOAD_T => $integer,
			CURLINFO_CONTENT_LENGTH_UPLOAD_T => $integer,
			CURLINFO_HTTP_VERSION => $integer,
			CURLINFO_PROTOCOL => $string,
			CURLINFO_PROXY_SSL_VERIFYRESULT => $integer,
			CURLINFO_SCHEME => $string,
			CURLINFO_SIZE_DOWNLOAD_T => $float,
			CURLINFO_SIZE_UPLOAD_T => $float,
			CURLINFO_SPEED_DOWNLOAD_T => $float,
			CURLINFO_SPEED_UPLOAD_T => $float,
			CURLINFO_APPCONNECT_TIME_T => $float,
			CURLINFO_CONNECT_TIME_T => $float,
			CURLINFO_FILETIME_T => $float,
			CURLINFO_NAMELOOKUP_TIME_T => $float,
			CURLINFO_PRETRANSFER_TIME_T => $float,
			CURLINFO_REDIRECT_TIME_T => $float,
			CURLINFO_STARTTRANSFER_TIME_T => $float,
			CURLINFO_TOTAL_TIME_T => $float,
		];

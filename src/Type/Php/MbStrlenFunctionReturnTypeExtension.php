	...
	): ?Type
	{
		...
		if (count($encodings) > 0) {
			for ($i = 0; $i < count($encodings); $i++) {
				if ($this->isSupportedEncoding($encodings[$i], $scope->getPhpVersion())) {
					continue;
				}
			...
			$encodings = array_merge($this->getSupportedEncodings($scope->getPhpVersion()), [self::UNSUPPORTED_ENCODING]);
		}
		...
		if (!$scope->getPhpVersion()->throwsOnInvalidMbStringEncoding()->yes() && in_array(self::UNSUPPORTED_ENCODING, $encodings, true)) {

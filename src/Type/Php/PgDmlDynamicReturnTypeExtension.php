...
final class PgDmlDynamicReturnTypeExtension implements DynamicFunctionReturnTypeExtension
{
	...
	): ?Type
	{
		...
		$getDefaultReturnType = static fn (): Type => ParametersAcceptorSelector::selectFromArgs(
			$scope,
			$args,
		...
		if (!isset($args[$flagsArgPosition])) {
			...
			return $this->getTypeFromFlags($functionName, TrinaryLogic::createNo(), TrinaryLogic::createYes(), $getDefaultReturnType);
		}
	...
	/**
	 * @param callable(): Type $getDefaultReturnType
	 */
	private function getTypeFromFlags(string $functionName, TrinaryLogic $containsString, TrinaryLogic $containsExec, callable $getDefaultReturnType): ?Type
	{
		if ($functionName === 'pg_insert') {
			...
			if ($containsExec->yes()) {
				return TypeCombinator::remove(
					$getDefaultReturnType(),
					new UnionType([new StringType(), new ConstantBooleanType(true)]),
				);
			}
			if (!$containsExec->no()) {
				return null;
		...
		return TypeCombinator::remove($getDefaultReturnType(), new StringType());
	}

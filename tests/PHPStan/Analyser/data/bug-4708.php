...
function GetASCConfig()
{
	...
	else
	{
		...
		assertType("non-empty-array<1|string>&hasOffsetValue('bsw', int)", $result);

		if (!isset($result['bew']))
		{
			assertType('non-empty-array<int|string>', $result);
			$result['bew'] = 5;
			assertType("non-empty-array<int|string>&hasOffsetValue('bew', 5)", $result);
		}
		else
		{
			assertType("non-empty-array<int|string>&hasOffsetValue('bew', int|string)", $result);
			$result['bew'] = (int) $result['bew'];
			assertType("non-empty-array<int|string>&hasOffsetValue('bew', int)", $result);
		}

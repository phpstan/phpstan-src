<?php

namespace ArraySpreadOperator;

use function PHPStan\Testing\assertType;

class Foo
{

	/**
	 * @param array<int, int> $integersArray
	 * @param array<int, int> $integersIterable
	 */
	public function doFoo(
		array $integersArray,
		iterable $integersIterable
	)
	{
		$integersOne = [1, 2, 3, ...$integersArray];
		$integersTwo = [1, 2, 3, ...$integersIterable];
		$integersThree = [1, 2, ...[3, 4, 5], 6, 7];

		$integersFour = array(1, 2, 3, ...$integersArray);
		$integersFive = array(1, 2, 3, ...$integersIterable);
		$integersSix = array(1, 2, ...[3, 4, 5], 6, 7);

		$integersSeven = array(1, 2, ...array(3, 4, 5), 6, 7);
		assertType('non-empty-list<int>', $integersOne);
		assertType('non-empty-list<int>', $integersTwo);
		assertType('array{1, 2, 3, 4, 5, 6, 7}', $integersThree);
		assertType('non-empty-list<int>', $integersFour);
		assertType('non-empty-list<int>', $integersFive);
		assertType('array{1, 2, 3, 4, 5, 6, 7}', $integersSix);
		assertType('array{1, 2, 3, 4, 5, 6, 7}', $integersSeven);
	}

}

...
function stringConcat($m, string $k): void
{
	if (is_array($m[$k . ".value"]) && $m[$k . ".value"]) {
		...
		assertType('non-empty-array<mixed, mixed>', $m[<<<HEREDOC
			$k.value
			HEREDOC]);
		assertType('mixed', $m[$k . '.other']);
	}
}

function interpolationEstablishesNarrowing($m, string $k): void
{
	if (is_array($m["$k.value"]) && $m["$k.value"]) {
		assertType('non-empty-array<mixed, mixed>', $m[$k . '.value']);
	}
...
function concatOfConstantStrings($m): void
{
	if (is_array($m['a' . 'b']) && $m['a' . 'b']) {
		assertType('non-empty-array<mixed, mixed>', $m['ab']);
		assertType('non-empty-array<mixed, mixed>', $m["ab"]);
		assertType('non-empty-array<mixed, mixed>', $m['a' . 'b']);
	}
}

function concatNesting($m, string $a, string $b): void
{
	if (is_array($m[$a . $b . '.value']) && $m[$a . $b . '.value']) {
		assertType('non-empty-array<mixed, mixed>', $m[$a . ($b . '.value')]);
		assertType('non-empty-array<mixed, mixed>', $m["$a$b.value"]);
		assertType('non-empty-array<mixed, mixed>', $m["$a" . "$b" . '.value']);
	}
}

function singlePartInterpolationIsAStringCast($m, int $i): void
{
	if (is_array($m["$i"]) && $m["$i"]) {
		assertType('non-empty-array<mixed, mixed>', $m["$i"]);
		assertType('non-empty-array<mixed, mixed>', $m["{$i}"]);
		// "$i" casts to string, $i does not - the offsets only coincide
		// because PHP normalizes integer-like string keys.
		assertType('mixed', $m[$i]);
	}
}

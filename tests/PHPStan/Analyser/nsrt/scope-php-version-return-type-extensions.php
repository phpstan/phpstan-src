...
function pregMatchUnmatchedAsNull(string $s): void
{
	if (PHP_VERSION_ID >= 70400) {
		if (preg_match('/(a)(b)?/', $s, $matches, PREG_UNMATCHED_AS_NULL) === 1) {
			assertType('array{non-falsy-string, \'a\', \'b\'|null}', $matches);
		}
	} else {
		if (preg_match('/(a)(b)?/', $s, $matches, PREG_UNMATCHED_AS_NULL) === 1) {
			assertType('array{0: non-falsy-string, 1: \'a\', 2?: \'b\'}', $matches);
		}
	}
}

function pregCaptureOnlyNamedGroups(string $s): void
{
	if (PHP_VERSION_ID >= 80200) {
		if (preg_match('/(?<name>a)(b)/n', $s, $named) === 1) {
			assertType('array{0: non-falsy-string, name: \'a\', 1: \'a\'}', $named);
		}
	} else {
		if (preg_match('/(?<name>a)(b)/n', $s, $named) === 1) {
			assertType('array{0: non-falsy-string, name: \'a\', 1: \'a\', 2: \'b\'}', $named);
		}
	}
}

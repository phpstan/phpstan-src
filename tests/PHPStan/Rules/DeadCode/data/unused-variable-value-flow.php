...
function superglobalOffsetIsSink(): void
{
	$value = source();
	$_GET['value'] = $value;
}

function superglobalAssignmentIsSink(): void
{
	$value = source();
	$_POST = ['value' => $value];
}

function flowIntoAliasedVariable(): void
{
	$reasons = [];
	$callback = function () use (&$reasons): array {
		return $reasons;
	};
	foreach ([1, 2] as $reason) {
		$reasons[] = $reason;
	}
	sink($callback);
}

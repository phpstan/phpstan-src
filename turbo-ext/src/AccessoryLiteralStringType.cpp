...
PT_MINIT_REGISTRATION(pt_register_accessory_literal_string_type)
{
	...
	cls.op(PT_OP_ACCEPTS, PT_OP_LAMBDA { return AccessoryLiteralStringType(self).accepts(argv, (Z_TYPE(argv[1]) == IS_TRUE)); });
	...
	cls.op<PT_OP_IS_SUPER_TYPE_OF, &AccessoryLiteralStringType::isSuperTypeOf>();
	...
	cls.op<PT_OP_IS_SUB_TYPE_OF, &AccessoryLiteralStringType::isSubTypeOf>();
	...
	cls.op(PT_OP_EQUALS, PT_OP_LAMBDA { return zv::Val::boolean(AccessoryLiteralStringType::equals(argv)); });

	cls.method(sigs::describe, [](INTERNAL_FUNCTION_PARAMETERS) {
		PT_ARGS(1, 1);
		RETURN_STRINGL("literal-string", sizeof("literal-string") - 1);
	});
	cls.op(PT_OP_DESCRIBE, PT_OP_LAMBDA { return zv::Val::string("literal-string", sizeof("literal-string") - 1); });
	...
	cls.op(PT_OP_HAS_OFFSET_VALUE_TYPE, PT_OP_LAMBDA { return pt_op_trinary(AccessoryLiteralStringType::hasOffsetValueType(argv)); });
	...
	cls.op<PT_OP_GET_OFFSET_VALUE_TYPE, &AccessoryLiteralStringType::getOffsetValueType>();

 ...
 * entries; InitializerExprTypeResolver::getBitwiseNotType() (which consults
...
pt_method_site pt_bnh_get_bitwise_not_type_site;
...
private:
	...
	static void typeCallbackBody(zval *captures, uint32_t argc, zval *argv, zval *return_value)
	{
		...
		pt_engine_with_stack([&]() {
			...
			type = pt_call_method_cached(pt_bnh_get_bitwise_not_type_site, Z_OBJ_P(OBJ_PROP_NUM(Z_OBJ(captures[0]), slots::initializerExprTypeResolver)), PT_LC("getbitwisenottype"), 2, callArgs);
		});

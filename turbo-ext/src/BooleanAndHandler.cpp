/*
 * PHPStanTurbo\BooleanAndHandler — native implementation of
 * PHPStan\Analyser\ExprHandler\BooleanAndHandler.
 *
 ...
 * callbacks it hands to BooleanNarrowingHelper::specifyConjunction() (the
 * operand result each).
 *
 ...
 * through their direct entries; the virtual BooleanAndNode is instantiated
 * through the class map.
 */

#include "support.h"
#include "generated/BooleanAndHandler.h"

namespace slots = ptdecl::BooleanAndHandler::slot;
namespace sigs = ptdecl::BooleanAndHandler::sig;
#include "OperatorHandlers.h"

zend_class_entry *pt_ce_boolean_and_handler = nullptr;

namespace phpstanturbo {

/* Mirrors PHPStan\Analyser\ExprHandler\BooleanAndHandler; UNDEF = pending
 * exception. */
class BooleanAndHandler
{
public:
	explicit BooleanAndHandler(zend_object *self) : self(self) {}
	...
	{
		int is = ptoh::isInstance(expr, PT_CLASS_BOOLEAN_AND_EXPR);
		if (UNEXPECTED(is < 0)) return false;
		if (!is) {
			is = ptoh::isInstance(expr, PT_CLASS_LOGICAL_AND_EXPR);
			if (UNEXPECTED(is < 0)) return false;
		}
		out = is == 1;
		return true;
	...
	zv::Val processExpr(zval *nodeScopeResolver, zval *stmt, zval *expr, zval *scope, zval *storage, zval *nodeCallback, zval *context) const
	{
		...
		zv::Val rightResult = pt_node_scope_resolver_process_expr_node(nodeScopeResolver, stmt, right, leftTruthyScope.raw(), storage, nodeCallback, rightContext.raw());
		...
		if (explicitNever) {
			...
			leftMergedWithRightScope = pt_mutating_scope_add_template_argument_constraints(Z_OBJ_P(leftFalseyScope.raw()), constraints.raw());
		} else {
		...
		zv::Args nodeArgv{expr, leftTruthyScope.raw()};
		zv::Val node = pt_type_new(PT_CLASS_BOOLEAN_AND_NODE, 2, nodeArgv);
	...
	{
		return BooleanAndHandler(handler).processExpr(nodeScopeResolver, stmt, expr, scope, storage, nodeCallback, context);
	}

private:
	zend_object *self;

	static constexpr const char *closureName = "PHPStan\\Analyser\\ExprHandler\\BooleanAndHandler::{closure}";
	...
	static zv::Val resolveType(zval *leftResult, zval *rightResult, bool nativeTypesPromoted)
	{
		...
		if (leftFalse) return ptoh::constantBoolean(false);
	...
	static void specifyTypesCallbackBody(zval *captures, uint32_t argc, zval *argv, zval *return_value)
	{
		...
		zv::Val rightFalseyScope = pt_native_closure(&falseyScopeBody, rightResult);
		zv::Val specifiedTypes = pt_boolean_narrowing_helper_specify_conjunction(Z_OBJ_P(OBJ_PROP_NUM(Z_OBJ(captures[0]), slots::booleanNarrowingHelper)), nodeScopeResolver, s, &argv[0], expr, left, leftTypes.raw(), leftTruthyScope.raw(), leftFalseyScope.raw(), right, rightTypes.raw(), rightFalseyScope.raw());
...
using phpstanturbo::BooleanAndHandler;

/* {{{ engine ABI glue: parameter parsing + registration */

#include "reg.h"

PT_MINIT_REGISTRATION(pt_register_boolean_and_handler)
{
	reg::Class cls("PHPStan\\Analyser\\ExprHandler\\BooleanAndHandler");
	ptdecl::BooleanAndHandler::declareClass(cls);
	ptdecl::BooleanAndHandler::declareProperties(cls);
	...
	cls.method(sigs::__construct, [](INTERNAL_FUNCTION_PARAMETERS) {
		...
		BooleanAndHandler(Z_OBJ_P(ZEND_THIS)).construct(booleanNarrowingHelper, expressionResultFactory);
	});
	...
	cls.method(sigs::supports, [](INTERNAL_FUNCTION_PARAMETERS) {
		...
		bool out;
		if (UNEXPECTED(!BooleanAndHandler::supports(expr, out))) RETURN_THROWS();
		RETURN_BOOL(out);
	});
	...
	cls.method(sigs::processExpr, [](INTERNAL_FUNCTION_PARAMETERS) {
		...
		PT_RETURN_VAL(BooleanAndHandler(Z_OBJ_P(ZEND_THIS)).processExpr(nodeScopeResolver, stmt, expr, scope, storage, nodeCallback, context));
	});

	cls.shadow(&pt_ce_boolean_and_handler);
	pt_expr_handler_entry_register(&pt_ce_boolean_and_handler, &BooleanAndHandler::processExprEntry);
}

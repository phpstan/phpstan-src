...
void pt_register_combinations_helper()
{
	...
	cls.method("combinations", reg::PublicStatic, 1, { reg::arrayArg("arrays") }, [](INTERNAL_FUNCTION_PARAMETERS) {
		/* the argument zval as passed: re-wrapping its table with ZVAL_ARR
		 * would mark an immutable [] refcounted, and the call's addref
		 * would write into the read-only zend_empty_array */
		zval *arrays;

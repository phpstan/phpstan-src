/*
 * PHPStanTurbo\NullsafeMethodCallHandler — native implementation of
 * PHPStan\Analyser\ExprHandler\NullsafeMethodCallHandler.
 *
 ...
 * $receiverType), the specifyTypesCallback ($this, $expr, $methodCall,
 ...
 * ($this, $expr, $methodCall, $exprResult, $receiverResult,
...
#include "support.h"
#include "generated/NullsafeMethodCallHandler.h"

namespace slots = ptdecl::NullsafeMethodCallHandler::slot;
namespace sigs = ptdecl::NullsafeMethodCallHandler::sig;
#include "CallHandlerSupport.h"

zend_class_entry *pt_ce_nullsafe_method_call_handler = nullptr;

namespace {

using namespace ptcall;

constexpr const char *pt_nmch_closure_name = "PHPStan\\Analyser\\ExprHandler\\NullsafeMethodCallHandler::{closure}";

/* {{{ the PHP collaborators (one site each; switch to their direct entries
 * once they are ported) */

pt_method_site pt_nmch_get_attributes_site;
...
{
	return pt_call_method_cached(pt_nmch_get_attributes_site, Z_OBJ_P(node), PT_LC("getattributes"), 0, NULL);
}
...
pt_property_site pt_nmch_var_site;
pt_property_site pt_nmch_name_site;
pt_property_site pt_nmch_args_site;

zval *exprVar(zval *expr) { return nodeProperty(pt_nmch_var_site, expr, PT_LC("var")); }
zval *exprName(zval *expr) { return nodeProperty(pt_nmch_name_site, expr, PT_LC("name")); }
zval *exprArgs(zval *expr) { return nodeProperty(pt_nmch_args_site, expr, PT_LC("args")); }
...
zend_string *pt_nmch_virtual_nullsafe_method_call = nullptr;
/* TemplateArgumentFrame::ORIGINAL_SITE_ATTRIBUTE */
zend_string *pt_nmch_original_site_attribute = nullptr;
zend_string *pt_nmch_null = nullptr;
/* ExprPrinter::ATTRIBUTE_CACHE_KEY */
zend_string *pt_nmch_cache_printer_attribute = nullptr;
...
/* Mirrors PHPStan\Analyser\ExprHandler\NullsafeMethodCallHandler; UNDEF =
 * pending exception. */
class NullsafeMethodCallHandler
{
public:
	explicit NullsafeMethodCallHandler(zend_object *self) : self(self) {}
	...
	{
		int is = isInstanceOf(expr, PT_CLASS_NULLSAFE_METHOD_CALL);
	...
	zv::Val processExpr(zval *nodeScopeResolver, zval *stmt, zval *expr, zval *scopeArg, zval *storage, zval *nodeCallback, zval *context) const
	{
		...
		zval *scopeBeforeNullsafe = scopeArg;
		...
		// carry the receiver type to NullsafeMethodCallRule
		{
			zv::Args nodeArgv{expr, receiverType.raw(), receiverNativeType.raw()};
			zv::Val node = pt_type_new(PT_CLASS_NULLSAFE_METHOD_CALL_EXPRESSION_NODE, 3, nodeArgv);
		...
		zv::Val methodCall;
		{
			...
			zv::Args callArgv{var, name, args, attributes.raw()};
			methodCall = pt_type_new(PT_CLASS_METHOD_CALL, 4, callArgv);
		...
		zv::Val exprResult;
		{
			zv::Val ensuredScope = ensuredResultScope(nonNullabilityResult.raw());
			if (UNEXPECTED(ensuredScope.isUndef())) return zv::Val();
			exprResult = pt_node_scope_resolver_process_expr_node_consuming_stored(nodeScopeResolver, stmt, methodCall.raw(), ensuredScope.raw(), storage, nodeCallback, context);
		...
		{
			if (UNEXPECTED(!receiverType.ref().isObject())) {
				zend_throw_error(NULL, "Call to a member function isNull() on %s", zend_zval_value_name(receiverType.raw()));
				return zv::Val();
			}
			zend_long varIsNull = isNullTrinary(receiverType.raw());
			if (UNEXPECTED(varIsNull < 0)) return zv::Val();
			if (varIsNull == PT_TRI_YES) {
				// arguments are never evaluated when the var is always null
				scope = zv::Val::copyOf(zv::Ref(scopeBeforeNullsafe));
			} else if (varIsNull == PT_TRI_MAYBE) {
				// arguments might not be evaluated (short-circuit): variables
				// assigned in them become "maybe defined"
				if (UNEXPECTED(!scope.ref().isObject())) {
					zend_throw_error(NULL, "Call to a member function mergeWith() on %s", zend_zval_value_name(scope.raw()));
					return zv::Val();
				}
				scope = pt_mutating_scope_merge_with(Z_OBJ_P(scope.raw()), scopeBeforeNullsafe);
				if (UNEXPECTED(scope.isUndef())) return zv::Val();
			}
		}
		...
		zv::Val specifyTypesCallback;
		{
			...
			ZVAL_COPY_VALUE(&captures[2], methodCall.raw());
		...
		zv::Val createTypesCallback = pt_native_closure(&createTypesCallbackBody, self, expr, methodCall.raw(), exprResult.raw(), receiverResult, nullsafeTypeCallback.raw(), beforeScope);
	...
	{
		return NullsafeMethodCallHandler(handler).processExpr(nodeScopeResolver, stmt, expr, scope, storage, nodeCallback, context);
	}

private:
	zend_object *self;

	/* $attributes = array_merge($expr->getAttributes(), ['virtualNullsafeMethodCall'
	 * => true, TemplateArgumentFrame::ORIGINAL_SITE_ATTRIBUTE => $expr]);
	...
	static zv::Val virtualAttributes(zval *expr)
	{
		...
		zv::Arr marker = zv::Arr::create(2);
		marker.set(pt_nmch_virtual_nullsafe_method_call, zv::Val::boolean(true));
		marker.set(pt_nmch_original_site_attribute, zv::Val::copyOf(zv::Ref(expr)));
		zv::Val attributes = arrayMerge(nodeAttributes.raw(), marker.raw());
		zval *raw = attributes.raw();
		if (zend_hash_exists(Z_ARRVAL_P(raw), pt_nmch_cache_printer_attribute)) {
			SEPARATE_ARRAY(raw);
			zend_hash_del(Z_ARRVAL_P(raw), pt_nmch_cache_printer_attribute);
		}
		return attributes;
	}
	...
	{
		if (UNEXPECTED(!requireArguments(argc, 1, pt_nmch_closure_name))) return;
	...
	{
		if (UNEXPECTED(!requireArguments(argc, 2, pt_nmch_closure_name))) return;
		zend_object *handler = Z_OBJ(captures[0]);
		zval *expr = &captures[1];
		zval *methodCall = &captures[2];
		...
		{
			zv::Val nullName = pt_type_new(PT_CLASS_NAME, 1, zv::Args{pt_nmch_null});
		...
		zv::Val conjunction = pt_boolean_narrowing_helper_specify_conjunction(Z_OBJ_P(booleanNarrowingHelper), nodeScopeResolver, s, context, expr, notIdenticalNode.raw(), leftTypes.raw(), leftTruthyScope.raw(), leftFalseyScopeCallback.raw(), methodCall, rightTypes.raw(), rightFalseyScope.raw());
	 ...
	 * $nativeTypesPromoted) use ($expr, $methodCall, $exprResult,
	 * $receiverResult, $nullsafeTypeCallback, $beforeScope): SpecifiedTypes —
	 * captures: $this, $expr, $methodCall, $exprResult, $receiverResult,
	...
	{
		if (UNEXPECTED(!requireArguments(argc, 3, pt_nmch_closure_name))) return;
		zval *defaultNarrowingHelper = OBJ_PROP_NUM(Z_OBJ(captures[0]), slots::defaultNarrowingHelper);
		zval *expr = &captures[1];
		zval *methodCall = &captures[2];
		zval *exprResult = &captures[3];
		...
		// !containsNull: the plain inner methodCall narrowed by $type, the
		// original ?-> key, and "receiver is not null"
		zv::Val plainTypes = pt_default_narrowing_helper_create_subject_types(defaultNarrowingHelper, s, methodCall, exprResult, type, context);
...
using phpstanturbo::NullsafeMethodCallHandler;
...
PT_MINIT_REGISTRATION(pt_register_nullsafe_method_call_handler)
{
	...
	pt_nmch_null = zend_string_init_interned(PT_LC("null"), 1);
	pt_nmch_cache_printer_attribute = zend_string_init_interned(PT_LC("phpstan_cache_printer"), 1);

	reg::Class cls("PHPStan\\Analyser\\ExprHandler\\NullsafeMethodCallHandler");
	ptdecl::NullsafeMethodCallHandler::declareClass(cls);
	ptdecl::NullsafeMethodCallHandler::declareProperties(cls);
	...
	cls.method(sigs::__construct, [](INTERNAL_FUNCTION_PARAMETERS) {
		...
		NullsafeMethodCallHandler(Z_OBJ_P(ZEND_THIS)).construct(nonNullabilityHelper, expressionResultFactory, defaultNarrowingHelper, booleanNarrowingHelper);
	});

	cls.method<&NullsafeMethodCallHandler::supports, zp::Obj>(sigs::supports);
	...
	cls.method(sigs::processExpr, [](INTERNAL_FUNCTION_PARAMETERS) {
		...
		PT_RETURN_VAL(NullsafeMethodCallHandler(Z_OBJ_P(ZEND_THIS)).processExpr(nodeScopeResolver, stmt, expr, scope, storage, nodeCallback, context));
	});

	cls.shadow(&pt_ce_nullsafe_method_call_handler);
	pt_expr_handler_entry_register(&pt_ce_nullsafe_method_call_handler, &NullsafeMethodCallHandler::processExprEntry);
}

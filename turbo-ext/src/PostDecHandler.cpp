/*
 * PHPStanTurbo\PostDecHandler — native implementation of
 * PHPStan\Analyser\ExprHandler\PostDecHandler.
 *
 * A DI service (#[AutowiredService]): the constructor keeps the twin's
 * arginfo so Nette autowires it. processExpr() is registered as the class's
 * handler entry (Engine.h). The synthetic pre-decrement's typeCallback is
...
#include "support.h"
#include "generated/PostDecHandler.h"

namespace slots = ptdecl::PostDecHandler::slot;
namespace sigs = ptdecl::PostDecHandler::sig;
#include "SimpleExprHandlers.h"

zend_class_entry *pt_ce_post_dec_handler = nullptr;

namespace {

using phpstanturbo::visitors::NodeProp;

NodeProp pt_podh_var = PT_NODE_PROP(PT_CLASS_POST_DEC, "var");

/* VariableWrite::KIND_POST_DEC */
constexpr zend_long pt_podh_kind = 6;

} // namespace

namespace phpstanturbo {

/* Mirrors PHPStan\Analyser\ExprHandler\PostDecHandler; UNDEF = pending
 * exception. */
class PostDecHandler
{
public:
	static constexpr const char *closureName = "PHPStan\\Analyser\\ExprHandler\\PostDecHandler::{closure}";
	static constexpr uint32_t defaultNarrowingHelperSlot = slots::defaultNarrowingHelper;

	explicit PostDecHandler(zend_object *self) : self(self) {}
	...
	{
		int is = ptoh::isInstance(expr, PT_CLASS_POST_DEC);
	...
	{
		zval *var = ptoh::operand(pt_podh_var, expr);
		if (UNEXPECTED(var == NULL)) return zv::Val();
		zv::Val valueFlowWrite = pt_variable_flow_builder_write_site(var, pt_podh_kind, scope, storage);
		...
		zv::Val virtualExpr = pt_type_new(PT_CLASS_PRE_DEC, 1, var);
		if (UNEXPECTED(virtualExpr.isUndef())) return zv::Val();
		zv::Val virtualTypeCallback = pt_inc_dec_type_helper_get_type_callback(OBJ_PROP_NUM(self, slots::incDecTypeHelper), var, varResult.raw(), false);
		if (UNEXPECTED(virtualTypeCallback.isUndef())) return zv::Val();
		zv::Val virtualSpecifyTypesCallback = pt_native_closure(&ptse::specifyDefaultTypesBody<PostDecHandler>, self, virtualExpr.raw());
		...
		zv::Val variableFlow = ptse::incDecFlow(child.variableFlow.raw(), valueFlowWrite.raw(), context, var, pt_podh_kind, assignedScope.raw(), storage);
		if (UNEXPECTED(variableFlow.isUndef())) return zv::Val();
		// post-decrement evaluates to the variable's pre-mutation value
		zv::Val typeCallback = pt_native_closure(&ptse::childTypeBody<PostDecHandler>, varResult.raw());
		zv::Val specifyTypesCallback = pt_native_closure(&ptse::specifyDefaultTypesBody<PostDecHandler>, self, expr);
	...
	{
		return PostDecHandler(handler).processExpr(nodeScopeResolver, stmt, expr, scope, storage, nodeCallback, context);
	}
...
using phpstanturbo::PostDecHandler;
...
void pt_register_post_dec_handler()
{
	...
	ptdecl::PostDecHandler::declareProperties(cls);
	...
	cls.method(sigs::__construct, [](INTERNAL_FUNCTION_PARAMETERS) {
		...
		PostDecHandler(Z_OBJ_P(ZEND_THIS)).construct(expressionResultFactory, defaultNarrowingHelper, incDecTypeHelper, assignHandler);
	});
	...
	cls.method(sigs::supports, [](INTERNAL_FUNCTION_PARAMETERS) {
		...
		bool out;
		if (UNEXPECTED(!PostDecHandler::supports(expr, out))) RETURN_THROWS();
		RETURN_BOOL(out);
	});
	...
	cls.method(sigs::processExpr, [](INTERNAL_FUNCTION_PARAMETERS) {
		...
		PT_RETURN_VAL(PostDecHandler(Z_OBJ_P(ZEND_THIS)).processExpr(nodeScopeResolver, stmt, expr, scope, storage, nodeCallback, context));
	});

	cls.shadow(&pt_ce_post_dec_handler);
	pt_expr_handler_entry_register(&pt_ce_post_dec_handler, &PostDecHandler::processExprEntry);
}

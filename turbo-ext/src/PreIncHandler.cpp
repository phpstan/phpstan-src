/*
 * PHPStanTurbo\PreIncHandler — native implementation of
 * PHPStan\Analyser\ExprHandler\PreIncHandler.
 *
...
#include "support.h"
#include "generated/PreIncHandler.h"

namespace slots = ptdecl::PreIncHandler::slot;
namespace sigs = ptdecl::PreIncHandler::sig;
#include "SimpleExprHandlers.h"

zend_class_entry *pt_ce_pre_inc_handler = nullptr;

namespace {

using phpstanturbo::visitors::NodeProp;

NodeProp pt_pih_var = PT_NODE_PROP(PT_CLASS_PRE_INC, "var");

/* VariableWrite::KIND_PRE_INC */
constexpr zend_long pt_pih_kind = 3;

} // namespace

namespace phpstanturbo {

/* Mirrors PHPStan\Analyser\ExprHandler\PreIncHandler; UNDEF = pending
 * exception. */
class PreIncHandler
{
public:
	static constexpr const char *closureName = "PHPStan\\Analyser\\ExprHandler\\PreIncHandler::{closure}";
	static constexpr uint32_t defaultNarrowingHelperSlot = slots::defaultNarrowingHelper;

	explicit PreIncHandler(zend_object *self) : self(self) {}
	...
	{
		int is = ptoh::isInstance(expr, PT_CLASS_PRE_INC);
	...
	{
		zval *var = ptoh::operand(pt_pih_var, expr);
		if (UNEXPECTED(var == NULL)) return zv::Val();
		zv::Val valueFlowWrite = pt_variable_flow_builder_write_site(var, pt_pih_kind, scope, storage);
		...
		zv::Val typeCallback = pt_inc_dec_type_helper_get_type_callback(OBJ_PROP_NUM(self, slots::incDecTypeHelper), var, varResult.raw(), true);
		if (UNEXPECTED(typeCallback.isUndef())) return zv::Val();
		zv::Val specifyTypesCallback = pt_native_closure(&ptse::specifyDefaultTypesBody<PreIncHandler>, self, expr);
		...
		zv::Val variableFlow = ptse::incDecFlow(child.variableFlow.raw(), valueFlowWrite.raw(), context, var, pt_pih_kind, assignedScope.raw(), storage);
	...
	{
		return PreIncHandler(handler).processExpr(nodeScopeResolver, stmt, expr, scope, storage, nodeCallback, context);
	}
...
using phpstanturbo::PreIncHandler;

/* {{{ engine ABI glue: parameter parsing + registration */

#include "reg.h"

PT_MINIT_REGISTRATION(pt_register_pre_inc_handler)
{
	reg::Class cls("PHPStan\\Analyser\\ExprHandler\\PreIncHandler");
	ptdecl::PreIncHandler::declareClass(cls);
	ptdecl::PreIncHandler::declareProperties(cls);
	...
	cls.method(sigs::__construct, [](INTERNAL_FUNCTION_PARAMETERS) {
		...
		PreIncHandler(Z_OBJ_P(ZEND_THIS)).construct(expressionResultFactory, incDecTypeHelper, defaultNarrowingHelper, assignHandler);
	});
	...
	cls.method(sigs::supports, [](INTERNAL_FUNCTION_PARAMETERS) {
		...
		bool out;
		if (UNEXPECTED(!PreIncHandler::supports(expr, out))) RETURN_THROWS();
		RETURN_BOOL(out);
	});
	...
	cls.method(sigs::processExpr, [](INTERNAL_FUNCTION_PARAMETERS) {
		...
		PT_RETURN_VAL(PreIncHandler(Z_OBJ_P(ZEND_THIS)).processExpr(nodeScopeResolver, stmt, expr, scope, storage, nodeCallback, context));
	});

	cls.shadow(&pt_ce_pre_inc_handler);
	pt_expr_handler_entry_register(&pt_ce_pre_inc_handler, &PreIncHandler::processExprEntry);
}

/*
 * PHPStanTurbo\SetExistingOffsetValueTypeExprHandler — native implementation of
 * PHPStan\Analyser\ExprHandler\Virtual\SetExistingOffsetValueTypeExprHandler.
 *
...
#include "support.h"
#include "generated/SetExistingOffsetValueTypeExprHandler.h"

namespace slots = ptdecl::SetExistingOffsetValueTypeExprHandler::slot;
namespace sigs = ptdecl::SetExistingOffsetValueTypeExprHandler::sig;
#include "VirtualExprHandlers.h"

zend_class_entry *pt_ce_set_existing_offset_value_type_expr_handler = nullptr;

namespace phpstanturbo {

/* Mirrors PHPStan\Analyser\ExprHandler\Virtual\SetExistingOffsetValueTypeExprHandler; UNDEF = pending
 * exception. */
class SetExistingOffsetValueTypeExprHandler
{
public:
	explicit SetExistingOffsetValueTypeExprHandler(zend_object *self) : self(self) {}
	...
	{
		return ptveh::supportsInstance(expr, PT_CLASS_SET_EXISTING_OFFSET_VALUE_TYPE_EXPR, out);
	}
	...
	zv::Val processExpr(zval *nodeScopeResolver, zval *stmt, zval *expr, zval *scope, zval *storage, zval *nodeCallback, zval *context) const
	{
		...
		zval *var = ptveh::getterRead(ptveh::setExistingOffsetValueTypeExprVar, expr, PT_LC("getvar"), varHold);
		...
		zval *dim = ptveh::getterRead(ptveh::setExistingOffsetValueTypeExprDim, expr, PT_LC("getdim"), dimHold);
		...
		zval *value = ptveh::getterRead(ptveh::setExistingOffsetValueTypeExprValue, expr, PT_LC("getvalue"), valueHold);
	...
	{
		return SetExistingOffsetValueTypeExprHandler(handler).processExpr(nodeScopeResolver, stmt, expr, scope, storage, nodeCallback, context);
	}

	static constexpr char closureName[] = "PHPStan\\Analyser\\ExprHandler\\Virtual\\SetExistingOffsetValueTypeExprHandler::{closure}";

private:
	zend_object *self;

	/* static fn (bool $nativeTypesPromoted): Type => ($nativeTypesPromoted ?
	 * $varResult->getNativeType() : $varResult->getType())->setExistingOffsetValueType(
	 * ($nativeTypesPromoted ? $dimResult->getNativeType() :
	 * $dimResult->getType()),
	...
	static void typeCallbackBody(zval *captures, uint32_t argc, zval *argv, zval *return_value)
	{
		...
		pt_engine_with_stack([&]() {
			...
			type = pt_type_call(Z_OBJ_P(varType.raw()), PT_LC("setexistingoffsetvaluetype"), 2, callArgs);
		});
...
using phpstanturbo::SetExistingOffsetValueTypeExprHandler;
...
PT_MINIT_REGISTRATION(pt_register_set_existing_offset_value_type_expr_handler)
{
	...
	ptdecl::SetExistingOffsetValueTypeExprHandler::declareProperties(cls);
	...
	cls.method(sigs::__construct, [](INTERNAL_FUNCTION_PARAMETERS) {
		...
		SetExistingOffsetValueTypeExprHandler(Z_OBJ_P(ZEND_THIS)).construct(expressionResultFactory);
	});
	...
	cls.method(sigs::supports, [](INTERNAL_FUNCTION_PARAMETERS) {
		...
		bool out;
		if (UNEXPECTED(!SetExistingOffsetValueTypeExprHandler::supports(expr, out))) RETURN_THROWS();
		RETURN_BOOL(out);
	});
	...
	cls.method(sigs::processExpr, [](INTERNAL_FUNCTION_PARAMETERS) {
		...
		PT_RETURN_VAL(SetExistingOffsetValueTypeExprHandler(Z_OBJ_P(ZEND_THIS)).processExpr(nodeScopeResolver, stmt, expr, scope, storage, nodeCallback, context));
	});

	cls.shadow(&pt_ce_set_existing_offset_value_type_expr_handler);
	pt_expr_handler_entry_register(&pt_ce_set_existing_offset_value_type_expr_handler, &SetExistingOffsetValueTypeExprHandler::processExprEntry);
}

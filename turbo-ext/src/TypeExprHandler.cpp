/*
 * PHPStanTurbo\TypeExprHandler — native implementation of
 * PHPStan\Analyser\ExprHandler\Virtual\TypeExprHandler.
 *
...
#include "support.h"
#include "generated/TypeExprHandler.h"

namespace slots = ptdecl::TypeExprHandler::slot;
namespace sigs = ptdecl::TypeExprHandler::sig;
#include "VirtualExprHandlers.h"

zend_class_entry *pt_ce_type_expr_handler = nullptr;

namespace phpstanturbo {

/* Mirrors PHPStan\Analyser\ExprHandler\Virtual\TypeExprHandler; UNDEF = pending
 * exception. */
class TypeExprHandler
{
public:
	explicit TypeExprHandler(zend_object *self) : self(self) {}
	...
	{
		return ptveh::supportsInstance(expr, PT_CLASS_TYPE_EXPR, out);
	}
	...
	{
		return TypeExprHandler(handler).processExpr(nodeScopeResolver, stmt, expr, scope, storage, nodeCallback, context);
	}
...
using phpstanturbo::TypeExprHandler;

/* {{{ engine ABI glue: parameter parsing + registration */

#include "reg.h"

PT_MINIT_REGISTRATION(pt_register_type_expr_handler)
{
	reg::Class cls("PHPStan\\Analyser\\ExprHandler\\Virtual\\TypeExprHandler");
	ptdecl::TypeExprHandler::declareClass(cls);
	ptdecl::TypeExprHandler::declareProperties(cls);
	...
	cls.method(sigs::__construct, [](INTERNAL_FUNCTION_PARAMETERS) {
		...
		TypeExprHandler(Z_OBJ_P(ZEND_THIS)).construct(virtualExprResultHelper);
	});
	...
	cls.method(sigs::supports, [](INTERNAL_FUNCTION_PARAMETERS) {
		...
		bool out;
		if (UNEXPECTED(!TypeExprHandler::supports(expr, out))) RETURN_THROWS();
		RETURN_BOOL(out);
	});
	...
	cls.method(sigs::processExpr, [](INTERNAL_FUNCTION_PARAMETERS) {
		...
		PT_RETURN_VAL(TypeExprHandler(Z_OBJ_P(ZEND_THIS)).processExpr(nodeScopeResolver, stmt, expr, scope, storage, nodeCallback, context));
	});

	cls.shadow(&pt_ce_type_expr_handler);
	pt_expr_handler_entry_register(&pt_ce_type_expr_handler, &TypeExprHandler::processExprEntry);
}

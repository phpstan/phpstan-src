/*
 * PHPStanTurbo\UnaryPlusHandler — native implementation of
 * PHPStan\Analyser\ExprHandler\UnaryPlusHandler.
 *
...
#include "support.h"
#include "generated/UnaryPlusHandler.h"

namespace slots = ptdecl::UnaryPlusHandler::slot;
namespace sigs = ptdecl::UnaryPlusHandler::sig;
#include "SimpleExprHandlers.h"

zend_class_entry *pt_ce_unary_plus_handler = nullptr;
...
/* Mirrors PHPStan\Analyser\ExprHandler\UnaryPlusHandler; UNDEF = pending
 * exception. */
class UnaryPlusHandler
{
public:
	static constexpr const char *closureName = "PHPStan\\Analyser\\ExprHandler\\UnaryPlusHandler::{closure}";
	static constexpr uint32_t defaultNarrowingHelperSlot = slots::defaultNarrowingHelper;

	explicit UnaryPlusHandler(zend_object *self) : self(self) {}
	...
	{
		int is = ptoh::isInstance(expr, PT_CLASS_UNARY_PLUS);
	...
	{
		zval *inner = ptoh::operand(pt_uph_expr, expr);
		...
		zv::Val specifyTypesCallback = pt_native_closure(&ptse::specifyDefaultTypesBody<UnaryPlusHandler>, self, expr);
	...
	{
		return UnaryPlusHandler(handler).processExpr(nodeScopeResolver, stmt, expr, scope, storage, nodeCallback, context);
	}
...
private:
	 ...
	 * $this->initializerExprTypeResolver->getUnaryPlusType($expr->expr, ...) —
...
using phpstanturbo::UnaryPlusHandler;
...
void pt_register_unary_plus_handler()
{
	...
	ptdecl::UnaryPlusHandler::declareProperties(cls);
	...
	cls.method(sigs::__construct, [](INTERNAL_FUNCTION_PARAMETERS) {
		...
		UnaryPlusHandler(Z_OBJ_P(ZEND_THIS)).construct(initializerExprTypeResolver, expressionResultFactory, defaultNarrowingHelper);
	});
	...
	cls.method(sigs::supports, [](INTERNAL_FUNCTION_PARAMETERS) {
		...
		bool out;
		if (UNEXPECTED(!UnaryPlusHandler::supports(expr, out))) RETURN_THROWS();
		RETURN_BOOL(out);
	});
	...
	cls.method(sigs::processExpr, [](INTERNAL_FUNCTION_PARAMETERS) {
		...
		PT_RETURN_VAL(UnaryPlusHandler(Z_OBJ_P(ZEND_THIS)).processExpr(nodeScopeResolver, stmt, expr, scope, storage, nodeCallback, context));
	});

	cls.shadow(&pt_ce_unary_plus_handler);
	pt_expr_handler_entry_register(&pt_ce_unary_plus_handler, &UnaryPlusHandler::processExprEntry);
}
